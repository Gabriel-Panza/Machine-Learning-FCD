...
def prepare_data_for_training_balanced(images_by_group, labels_by_group, train_size=0.7, validation_size=0.2, test_size=0.1):
    ...
    for patient_id in images_by_group:
        ...
        # Concatenar as amostras balanceadas para este paciente
        balanced_images_by_patient[patient_id] = class_1_images + class_0_images_sampled
        balanced_labels_by_patient[patient_id] = class_1_labels + class_0_labels_sampled
...
history = cnn_model.fit(X_train, y_train, validation_data=(X_valid, y_valid), batch_size=32, epochs=200, callbacks=[early_stopping])

# Avaliar o modelo
y_pred = (cnn_model.predict(X_test) >  0.5)
