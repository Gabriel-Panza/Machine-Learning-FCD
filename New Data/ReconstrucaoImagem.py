...
def build_image(img, mask):
    ...
    mascara_reconstruida = np.zeros((4 * patch_size, 4 * patch_size), dtype=np.uint8)
    ...
    for i in range(4):  # Linha
        for j in range(4):  # Coluna
            ...
            imagem_reconstruida[x_start:x_start + patch_size, y_start:y_start + patch_size] = img[idx]
            mascara_reconstruida[x_start:x_start + patch_size, y_start:y_start + patch_size] = mask[idx]
